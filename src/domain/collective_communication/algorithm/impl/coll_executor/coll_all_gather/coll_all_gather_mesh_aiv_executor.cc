...
namespace hccl {
CollAllGatherMeshAivExecutor::CollAllGatherMeshAivExecutor(const HcclDispatcher dispatcher,
...
HcclResult CollAllGatherMeshAivExecutor::CalcStreamNum(u32& streamNum)
{
    streamNum = 0; // AIV通信不需要申请从流
    HCCL_INFO("[CollAllGatherMeshAivExecutor][CalcStreamNum] tag[%s] streamNum[%u].",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}
 
HcclResult CollAllGatherMeshAivExecutor::GetIfNeedAivBuffer(bool &needAivBuffer)
{
    // AIV通信需要AIV buffer
    needAivBuffer = true;
    HCCL_INFO("[CollAllGatherMeshAivExecutor][GetIfNeedAivBuffer]tag[%s] needAivBuffer is [%u].",
        tag_.c_str(), needAivBuffer);
    return HCCL_SUCCESS;
}
 
HcclResult CollAllGatherMeshAivExecutor::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult CollAllGatherMeshAivExecutor::CalcTransportMemType(TransportMemType &inputType,
    TransportMemType &outputType)
{
    ...
    HCCL_INFO("[CollAllGatherMeshAivExecutor][CalcTransportMemType] tag[%s] inputType[%d],"
        " outputType[%d].", tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}
 
HcclResult CollAllGatherMeshAivExecutor::CalcLevel0CommInfo(TransportMemType inputType,
    TransportMemType outputType,
...
HcclResult CollAllGatherMeshAivExecutor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
...
HcclResult CollAllGatherMeshAivExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    HCCL_DEBUG("[CollAllGatherMeshAivExecutor][KernelRun] userRank [%d] localRank [%d]",
...
REGISTER_EXEC("AllGatherMeshAivExecutor", AllGatherMeshAiv, CollAllGatherMeshAivExecutor);
