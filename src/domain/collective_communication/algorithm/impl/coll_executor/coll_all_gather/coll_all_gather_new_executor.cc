...
{
    HCCL_INFO("[CollAllGatherNewExecutor][CalcLevel0CommInfo]tag[%s] start", tag_.c_str());
    CommParaInfo commParaLevel0(COMM_LEVEL0, CommType::COMM_TAG_RING_INNER);
    CHK_RET(CalcCommPlaneInfo(tag_, commParaLevel0, opTransport[COMM_LEVEL0], inputType, outputType));
    HCCL_INFO("[CollAllGatherNewExecutor][CalcLevel0CommInfo]tag[%s] Calc RingComm finish", tag_.c_str());
    return HCCL_SUCCESS;
}
...
{
    HCCL_INFO("[CollAllGatherNewExecutor][KernelRun]The AllGatherNewExecutor starts.");
    u32 perDataSize = 0;
    CHK_RET(SalGetDataTypeSize(param.DataDes.dataType, perDataSize));
    CHK_PRT_RET(perDataSize == 0,
        HCCL_ERROR("[CollAllGatherNewExecutor][KernelRun]errNo[0x%016llx] datatype[%d] is invalid",
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAllGatherNewExecutor][KernelRun]all gather 8PringHD memcpy Failed, "
    ...
    CHK_PRT_RET(multRingsSliceZero.size() != ringNum,
        HCCL_ERROR("[CollAllGatherNewExecutor][KernelRun]ringNum[%u] != multRingsSliceZero size[%zu]",
    ...
    CHK_PRT_RET(hdCount == 0,
        HCCL_ERROR("[CollAllGatherNewExecutor][KernelRun]hdCount[%llu] is invalid", hdCount), HCCL_E_PARA);
