...
__aicore__ inline void AivReduceScatterSmall91093::Process(GM_ADDR input, GM_ADDR output, uint64_t len, int32_t tag)
{
    ...
    // 共用16个flag
    uint32_t flagOffsetBase = BASE_FLAG_OFFSET * AIV_REDUCE_SCATTER_91093_SMALLDATA;
    ...
    if (dstRank != rank_) {
        ...
        globalSet.SetGlobalBuffer((__gm__ int32_t *)(GM_OUT[rank_] + flagOffset + blockIdxInGroup * FLAG_SIZE + dstRank * blockNumPerGroup * FLAG_SIZE), UB_FLAG_PAD_COUNT);
        ...
        globalCheck.SetGlobalBuffer((__gm__ int32_t *)(GM_OUT[dstRank] + flagOffset + blockIdxInGroup * FLAG_SIZE + rank_ * blockNumPerGroup * FLAG_SIZE), UB_FLAG_PAD_COUNT);
        while (true) {
        ...
        globalCheck.SetGlobalBuffer((__gm__ int32_t *)(GM_OUT[rank_] + flagOffset + rank_ * blockNumPerGroup * FLAG_SIZE + blockIdxInGroup * FLAG_SIZE), UB_FLAG_PAD_COUNT);
        while (true) {
            DataCopy(localFlagTensor[8], globalCheck, UB_FLAG_PAD_COUNT);
            SyncFunc<HardEvent::MTE2_S>();
            if (localFlagTensor[8].GetValue(0) == tag) {
        ...
        // 卡内同步
        GlobalTensor<int32_t> globalSet;
        globalSet.SetGlobalBuffer((__gm__ int32_t *)(GM_OUT[rank_] + flagOffset + rank_ * blockNumPerGroup * FLAG_SIZE + blockIdxInGroup * FLAG_SIZE), UB_FLAG_PAD_COUNT);
        localFlagTensor.SetValue(0, tag);
        PipeBarrier<PIPE_MTE3>();
        SyncFunc<HardEvent::S_MTE3>();
        DataCopy(globalSet, localFlagTensor, UB_FLAG_PAD_COUNT);
